import {ValidatorError} from "../../../domain/errors/validator-error"
...
describe("ValidatorRules Unit Tests", () => {
  ...
  test("required validation rule", () => {
    ...
    const error = new ValidatorError("The field is required")
  ...
  test("boolean validation rule", () => {
    ...
    const error = new ValidatorError("The field must be a boolean")
    arrange.forEach((item) => {
      assertIsInvalid({
        value: item.value,
        property: item.property,
  ...
  it("should throw a validation error when combine two or more validation rules", () => {
    ...
    }).toThrow(new ValidatorError("The field is required"))

    validator = ValidatorRules.values(5, "field")
    expect(() => {
      validator.required().string().maxLength(5)
    }).toThrow(new ValidatorError("The field must be a string"))
    ...
    expect(() => {
      validator.required().boolean()
    }).toThrow(new ValidatorError("The field must be a boolean"))
  })
